...
class TestALNS(TestCase):
    ...
    def test_solve(self):
        ...
        cost_function = lambda x: -len(x)
        destroy_function = lambda x, q: (len(x) - q) * [1] if len(x) > q else []
        repair_function = lambda x, q: (len(x) + q) * [1]
        completeness_function = lambda x: len(x) == 5
