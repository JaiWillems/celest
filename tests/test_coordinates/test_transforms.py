...
class TestCoordinateTransformations(TestCase):
    ...
    def test_itrs_to_wgs84_for_validation(self):
        ...
        self.assertTrue(np.allclose(
            wgs84.latitude.to(u.deg),
            self.wgs84.latitude.to(u.deg),
            atol=0.18
        ))
        self.assertTrue(np.allclose(
            wgs84.longitude.to(u.deg),
            self.wgs84.longitude.to(u.deg),
            atol=0.00001
        ))
        self.assertTrue(np.allclose(
            wgs84.height.to(u.km),
            self.wgs84.height.to(u.km),
            atol=0.06
        ))
    ...
    def test_itrs_to_azel_for_validation(self):
        ...
        self.assertTrue(np.allclose(
            expected_azel.alt.degree,
            actual_azel.elevation.data,
            atol=0.32
        ))
        self.assertTrue(np.allclose(
            expected_azel.az.degree,
            actual_azel.azimuth.data,
            atol=7.3
        ))
